...
Contact is modeled by a one-sided linear penalty:
$f_c = k_c |g_i| L_0$ for $g_i < 0$, where $g_i$ is the gap at node $i$ and
$k_c = C/R_0$.  A 2-point Gauss quadrature along each edge pair accurately
represents distributed contact pressure across the flattened contact zone.
...
whose angular frequency is~\cite{Lamb1932}
\begin{equation}
  \omega_2 = \sqrt{\frac{n(n^2-1)\gamma}{\rho_d R_0^3}}\bigg|_{n=2}
           = \sqrt{\frac{6\gamma}{\rho_d R_0^3}}
           = \sqrt{6} \approx 2.449\;\tau_0^{-1}.
  \label{eq:omega2}
\end{equation}
This mode is area-conserving to leading order for a continuous circle, making it an
ideal probe of line-tension mechanics independent of the area-penalty parameter.
...
\begin{equation}
  \omega_2^{\rm meas} = 2.731\;\tau_0^{-1} \quad
  (11.5\%\ \text{above continuum theory}).
\end{equation}
The overestimate is a geometric property of the discrete polygon: the $n=2$
perturbation of an $N$-node polygon slightly mixes area and shape deformation
(unlike the continuum circle, where the mode is exactly area-conserving).  The
correction is independent of $N$ for $N \geq 32$, confirming it reflects the
polygon topology rather than spatial discretization error.
...
\approx 5.46$.  Figure~\ref{fig:emulsion_capwave}b verifies the amplitude
...
\begin{figure}[htb]
  ...
  \caption{%
    ...
    $\alpha_{\rm damp}^{\rm crit} = 2\omega_2^{\rm meas} \approx 5.46$.
