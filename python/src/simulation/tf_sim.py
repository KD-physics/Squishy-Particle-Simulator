...
def inter_capsule_forces_tf(x_all, CapCandidates, r_c_flat, k_c_flat, L0_flat,
    ...
    for sg, w0 in [(sg0, _one - sg0), (sg1, _one - sg1)]:
        ...
        f_out = f_out + w0 * F_src                             # a0: global node k

        # a1 contribution: scatter sg * F_src to a1_global[k]
        f_out = tf.tensor_scatter_nd_add(
            f_out, tf.expand_dims(a1_global, 1), sg * F_src)

        # Newton 3rd — scatter to candidate (B) edge endpoints
        ...
        f_out = tf.tensor_scatter_nd_add(
            f_out,
            tf.reshape(cand_0safe, [K * E, 1]),
            -(_one - t_B_flat) * F_vec_flat)

        # b1 nodes
        f_out = tf.tensor_scatter_nd_add(
            f_out,
            tf.reshape(b1_global, [K * E, 1]),
            -t_B_flat * F_vec_flat)
...
def step_full_tf(state, CapCandidates, dt, alpha_damp, g, params,
    ...
    return step_rb_tf(state, f_contact, dt, alpha_damp, g, params, t=t)
...
def run_simulation_tf(state0, dt, alpha_damp, g, params, n_steps, mgr_cpp,
    ...
    def loop_body(state, CapCand, step_idx):
        ...
        new_state, _ = step_full_tf(
            state, new_cand, dt_tf, alpha_tf, g_tf, params,
            t=t, prim_data=prim_data)

        return new_state, new_cand, step_idx + 1

    def loop_cond(state, CapCand, step_idx):
        return step_idx < step_end

    step_init = tf.constant(step_offset, dtype=tf.int32)

    final_state, _, _ = tf.while_loop(
        loop_cond,
        loop_body,
        loop_vars=(state0, CapCand_init, step_init),
