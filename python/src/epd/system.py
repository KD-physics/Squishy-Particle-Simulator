...
class System:
    ...
    def initialize(self, phi_target=0.80, seed=42, verbose=True,
        ...
        if p0.m_node > 0:
            ...
            om_max = om_edge
        dt_val = float(self._dt_factor * 2.0 / om_max)
...
# ── stepping ──────────────────────────────────────────────────────────────
    ...
    def make_movie(self, output_path, fps=10, n_arc=6,
        ...
        """
        Render an animated movie (GIF) from self.frames and self.callback_data.
        ...
        if not self.frames:
            raise RuntimeError("No frames recorded; call run() first.")

        out_path = pathlib.Path(output_path)
        out_path.parent.mkdir(parents=True, exist_ok=True)

        snap_frames = self.frames          # list of snapshot dicts
        cb          = self.callback_data   # list of callback return dicts
        ...
        for obj in self._objects:
            ...
            for ki, pd in enumerate(obj.resolved(t=t0)):
                ...
                if isinstance(prim, LineSegment):
                    ...
                    circ = mpatches.Circle(
                        prim.center, prim.radius,
                        fill=False, ec=color, lw=lw, alpha=alpha, zorder=1,
                        linestyle='-' if not prim.convex else '--')
                    ax_sim.add_patch(circ)
                    obj_artists.append((obj, circ, 'arc', ki))
