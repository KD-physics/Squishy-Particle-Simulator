"""
test_A_emulsion_api.py — Test A (emulsion): Two-droplet oscillatory squeeze via System API

Uses identical physics parameters and geometry to test_A_emulsion_tf.py (ground truth).
Machine-precision agreement verified by 10-step cross-check at startup.

Usage:
    python src/validation/test_A_emulsion_api.py [--cycles N] [--out PATH]
"""
...
parser.add_argument('--out',    type=str, default='results/test_A_emulsion_api.gif')
...
R0        = 1.0
gamma     = 1.0
K_area    = 50.0      # kappa = gamma/(R0*K_area) = 0.02
C         = 500.0
k_reg     = 10.0
rho_d     = 1.0
alpha_damp = 5.0
...
print("=" * 65)
print(f"Test A (emulsion API): Two-droplet oscillatory squeeze  (kappa={kappa:.3f})")
print("=" * 65)
...
# Emulsion: r_c_wall=0 (contact radius lives on droplet only)
...
def _prim_direct(t_now):
    ...
    for sg in segs: sg.r_c = 0.0
    return make_prim_data([(sg, 1., np.zeros(2), sg.r_c, 0., np.zeros(2))
                           for sg in segs])

_p1c = EmulsionParticle(N=N, R0=R0, gamma=gamma, K_area=K_area,
                         C=C, rho_d=rho_d, k_reg=k_reg, center=(0.0, -cy_drop))
_p2c = EmulsionParticle(N=N, R0=R0, gamma=gamma, K_area=K_area,
                         C=C, rho_d=rho_d, k_reg=k_reg, center=(0.0,  cy_drop))
...
_sys_chk.add_object(OscillatingWall(y_top0, half_w, A_wall, omega_osc,
                                     is_top=True,  r_c_wall=0.0))
_sys_chk.add_object(OscillatingWall(y_bot0, half_w, A_wall, omega_osc,
                                     is_top=False, r_c_wall=0.0))
_lw = Wall([-half_w,-y_top0*2],[-half_w,y_top0*2],[+1,0]); _lw.set_r_c_wall(0.0)
_rw = Wall([ half_w,-y_top0*2],[ half_w,y_top0*2],[-1,0]); _rw.set_r_c_wall(0.0)
_sys_chk.add_object(_lw); _sys_chk.add_object(_rw)
_p1d = EmulsionParticle(N=N, R0=R0, gamma=gamma, K_area=K_area,
                         C=C, rho_d=rho_d, k_reg=k_reg, center=(0.0, -cy_drop))
_p2d = EmulsionParticle(N=N, R0=R0, gamma=gamma, K_area=K_area,
                         C=C, rho_d=rho_d, k_reg=k_reg, center=(0.0,  cy_drop))
...
def squeeze_frame(sys_obj):
    ...
    t_norm  = t_now / T_osc
    A1 = _area_poly(x1); A2 = _area_poly(x2)
    ...
    return {
        ...
        'text'       : (f"t/T={t_norm:.2f}  ε_wall={ws:.3f}\n"
                        f"ε_p={eps_avg:.3f}  1-A/A₀={area_val:.5f}\n"
                        f"cc_gap={cc_gap:+.4f}"),
    }
...
print(f"  [{'PASS' if ok_area   else 'FAIL'}] area conservation  "
      f"(1-A/A0 < 8*kappa+0.5%={8*kappa+0.005:.4f}: actual {max_dA:.4f})")
print(f"  [{'PASS' if ok_contact else 'FAIL'}] droplets make contact  "
...
# ── Render movie ──────────────────────────────────────────────────────────────
sys_api.make_movie(
    args.out, fps=10, n_arc=4,
    title=f'Two-droplet oscillatory squeeze  (kappa={kappa:.2f}, 10% strain)  [API tf-fast]',
)

all_pass = ok_strain and ok_area and ok_contact and mp_ok and plumbing_ok
print(f"\n{'PASS' if all_pass else 'FAIL'}: Test A (emulsion, API tf-fast)")
