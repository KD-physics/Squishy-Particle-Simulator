...
omega2_analytic = np.sqrt(8.0 * GAMMA / (RHO_D * R0**3))  # √8 ≈ 2.828
# n=2 capillary mode of a 2D liquid disk under purely radial perturbation
# (compressible / no internal area-preservation): ω_n² = 2 n² γ/(ρR³).
# Lamb's incompressible formula ω_n² = n(n²-1)γ/(ρR³) gives √6 at n=2 — that
# result requires the boundary motion to enforce internal-area conservation
# via a coupled n=0 breathing component (Lamb, Hydrodynamics §271); the
# purely-radial perturbation u_r = εR cos(2θ) used in this benchmark does
# not impose that constraint and so realises the larger ω² = 8γ/(ρR³).
...
def _perturb_x(x_all, x_cm, eps=0.05):
    ...
    Returns (x_out, u_disp) where:
      x_out  : (P, N, 2) — perturbed lab-frame node positions
      u_disp : (P, N, 2) — radial displacement vectors (lab frame)

    Callers must set BOTH state['x_all'] = x_out AND state['u'] = u_disp
    (rotated to body frame if theta != 0). Setting only x_all loses the
    perturbation on the first integration step because the rigid-body
    integrator reconstructs x from u via x = R(θ)·(X_ref + u) + x_cm.
    """
    P = x_all.shape[0]
    x_out  = x_all.copy()
    u_disp = np.zeros_like(x_all)
    for p in range(P):
...
def run_capwave(alpha_damp, n_periods=10, eps=0.05, n_sample=300):
    """
    ...
    # Must set BOTH x_all and u (elastic displacement); the integrator
    # reconstructs x from u, so setting x_all alone loses the perturbation.
    x0    = sys_._state['x_all'].numpy()
    xcm0  = sys_._state['x_cm'].numpy()
    x0p, u_disp = _perturb_x(x0, xcm0, eps=eps)
    sys_._state['x_all'] = tf.constant(x0p,    dtype=tf.float64)
    sys_._state['u']     = tf.constant(u_disp, dtype=tf.float64)
    ...
    sample_every = max(1, n_tot // n_sample)
...
def run_capwave_batch(alphas, n_periods=10, eps=0.05, n_sample=300):
    """
    Batched mode-2 ringdown: evaluate multiple alpha_damp values in a single
    TF run by placing one droplet per alpha into a single System.

    Exploits GPU parallelism over particles — all alphas evolve in one
    tf.while_loop pass at the cost of 1 alpha. Useful as a user example of
    how to leverage the per-particle vector parameters of System for a sweep.

    Returns (ts, ells) where ells has shape (P, T): one ellipticity time
    series per alpha (in the same order as `alphas`).
    """
    P  = len(alphas)
    spec = ParticleSpec(count=P, type='emulsion', gamma=GAMMA, kappa=KAPPA,
                        N_nodes=N, Oh=None)
    # Generous box so RSA placement gives well-separated droplets that don't
    # come into contact during the perturbation.
    Lx = max(20.0, 6.0 * R0 * P)
    sys_ = System(Lx, 20.0, periodic_x=False, periodic_y=False, g=0.0)
    sys_.add_particles(spec)
    sys_.initialize(phi_target=0.05, seed=42, verbose=False,
                    relax_only=True, n_relax_init=0)

    # Apply the same n=2 radial perturbation to every droplet (around its own CM).
    # Set BOTH x_all and u (elastic displacement); the integrator reconstructs
    # x from u, so setting x_all alone loses the perturbation.
    x0    = sys_._state['x_all'].numpy()
    xcm0  = sys_._state['x_cm'].numpy()
    x0p, u_disp = _perturb_x(x0, xcm0, eps=eps)
    sys_._state['x_all'] = tf.constant(x0p,    dtype=tf.float64)
    ...
    # Per-particle damping — this is the key: one alpha per particle
    sys_._params['alpha_damp_per_p'] = tf.constant(np.asarray(alphas, dtype=np.float64))

    ts_out, ell_out = [], []
    def _cb(sys_obj):
        snap = sys_obj.snapshot()
        ts_out.append(snap['t'])
        ell_out.append(_ellipticity_from_x(snap['x_all'], snap['x_cm']).copy())
        return {}
    ...
    ts   = np.array(ts_out)
    ells = np.array(ell_out)         # shape (T, P)
    return ts, ells.T                 # shape (P, T)
...
def run_benchmark_C():
    print("\n── Benchmark C: Capillary-wave damping (batched) ─────────────────")
    alphas = [0.5, 1.0, 2.0, 5.0, 10.0]
    colors = ['#1f77b4', '#ff7f0e', '#2ca02c', '#d62728', '#9467bd']

    # All 5 alpha values evolve in a single batched TF run — one droplet per alpha
    print(f"  Running {len(alphas)} alpha values as a single batched System...")
    ts, ells_per_alpha = run_capwave_batch(alphas, n_periods=12, eps=0.05, n_sample=300)
    omega2_meas = _measure_omega(ts, ells_per_alpha[0])   # frequency from α=0.5 (most underdamped)
    ...
    all_data = {a: (ts, ells_per_alpha[i]) for i, a in enumerate(alphas)}
