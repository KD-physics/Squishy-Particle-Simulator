"""
test_A_emulsion_tf.py — Test A (emulsion): Two-droplet oscillatory squeeze via C++/TF backend

Geometry and rendering identical to test_A_emulsion_oscillate.py.
Integration uses step_full_tf + CandidacyManager instead of NumPy per-node Euler.

Note: k_reg (tangential regularization) is not implemented in internal_forces_tf;
      for a 3-cycle test node bunching is negligible.

Usage:
    python src/validation/test_A_emulsion_tf.py [--cycles N] [--out PATH]
"""
...
# ── geometry helpers (identical to NumPy emulsion test) ───────────────────────

def _capsule_outline_polygon(x, r_c, n_arc=4):
    N  = len(x)
...
class OscillatingWall:
    """r_c_wall = 0 for emulsion (contact radius lives on droplet only)."""
    def __init__(self, is_top, y0, half_w, A, omega):
        self.is_top = is_top
        self.y0     = float(y0)
    ...
    def wall_strain(self):
        return abs(self.y - self.y0)


# ── CLI ───────────────────────────────────────────────────────────────────────
parser = argparse.ArgumentParser()
parser.add_argument('--cycles', type=int, default=3)
parser.add_argument('--out',    type=str, default='results/test_A_emulsion_tf.gif')
args = parser.parse_args()

# ── canonical κ=0.02 parameters (identical to NumPy test) ─────────────────────
N         = 32
R0        = 1.0
gamma     = 1.0
K_area    = 50.0     # κ = γ/(R0·K_area) = 0.02
C         = 500.0
k_reg     = 10.0
rho_d     = 1.0
alpha_damp = 5.0
...
print("=" * 65)
print(f"Test A (emulsion TF): Two-droplet oscillatory squeeze  (κ={kappa:.3f})")
print("=" * 65)
print(f"  κ={kappa:.3f}  K_area={K_area}  C={C}  k_reg={k_reg}")
print(f"  c_cap={c_cap:.2f}  dt={dt:.2e}  T_cap={T_cap:.4f}  α={alpha_damp}")
print(f"  Oscillation: A={A_wall}  T_osc={T_osc:.4f}  ω={omega_osc:.4f}")
print(f"  Total: {n_cycles} cycles  {n_steps} steps  t_total={t_total:.3f}")

# ── build particles (identical to NumPy test) ─────────────────────────────────
...
print(f"  peak |1-A/A₀|   : {max_dA:.6f}  (κ={kappa:.2f} → expect < 8κ = {8*kappa:.2f})")
...
print(f"  [{'PASS' if ok_area   else 'FAIL'}] area conservation  "
      f"(1-A/A₀ < 8κ+0.5% = {8*kappa+0.005:.4f}: actual {max_dA:.4f})")
print(f"  [{'PASS' if ok_contact else 'FAIL'}] droplets make contact  (min_cc_gap = {min_cc_gap:.4e})")

# ── render GIF (identical structure to NumPy emulsion test) ───────────────────
import pathlib
...
ax_sim.set_title(f'Two-droplet oscillatory squeeze  (κ={kappa:.2f}, 10% strain)  [TF]')

wall_kw = dict(fc='#888888', ec='#555555', lw=0.5, alpha=0.85, zorder=1)

ax_sim.add_patch(mpatches.Rectangle(
    (-lim_x, -lim_y * 1.5), lim_x - half_w_f, lim_y * 3.0, **wall_kw))
ax_sim.add_patch(mpatches.Rectangle(
    (half_w_f, -lim_y * 1.5), lim_x - half_w_f, lim_y * 3.0, **wall_kw))
...
fill1 = MplPolygon(np.zeros((4, 2)), closed=True,
                    fc='steelblue', ec='navy', lw=0.8, alpha=0.55, zorder=3)
fill2 = MplPolygon(np.zeros((4, 2)), closed=True,
                    fc='tomato', ec='darkred', lw=0.8, alpha=0.55, zorder=3)
...
dA_arr     = np.array([1.0 - 0.5 * (f['A1'] + f['A2']) / f['A0'] for f in frames])

l_strain, = ax_ts.plot([], [], 'b-',  lw=1.5, label='wall strain')
l_eps,    = ax_ts.plot([], [], 'r-',  lw=1.5, label='ε_particle (avg)')
l_dA,     = ax_ts.plot([], [], 'g--', lw=1.0, label='1-A/A₀ (×10)')
...
def _update(i):
    ...
    top_patch.set_xy((-lim_x, y_t))
    top_patch.set_height(lim_y - y_t)
    bot_patch.set_xy((-lim_x, -lim_y))
    bot_patch.set_height(y_b - (-lim_y))
    ...
    l_dA.set_data(t_arr[:i+1]     / T_osc, dA_arr[:i+1] * 10)
    ...
    txt.set_text(
        ...
        f"ε_p={eps_avg:.3f}\n"
        f"1-A/A₀={1-0.5*(fr['A1']+fr['A2'])/fr['A0']:.5f}\n"
        f"cc_gap={fr['cc_gap']:+.4f}"
    )
    return [top_patch, bot_patch, fill1, fill2, l_strain, l_eps, l_dA, vline, txt]
...
if ok_strain and ok_area and ok_contact:
    print(f"\nPASS: Test A (emulsion, TF)")
else:
    print(f"\nFAIL: Test A (emulsion, TF)")
